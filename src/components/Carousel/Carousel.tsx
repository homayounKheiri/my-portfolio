...
export default function Carousel({
    ...
    if (autoplay && (!pauseOnHover || !isHovered)) {
      const timer = setInterval(() => {
        setCurrentIndex((prev) => {
          if (prev === children.length - 1 && loop) {
            return prev + 1;
          }
  ...
  const handleDragEnd = (
    ...
    if (offset < -DRAG_BUFFER || velocity < -VELOCITY_THRESHOLD) {
      if (loop && currentIndex === children.length - 1) {
  ...
  return (
    ...
    >
      ...
      {showDots && (
        ...
        >
          <div className="mt-4 mb-2 flex w-[150px] justify-between px-8">
            {children.map((_, index) => (
              <motion.div
                ...
                animate={{
                  scale: currentIndex % children.length === index ? 1.2 : 1,
                }}
